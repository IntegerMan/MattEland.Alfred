...
public abstract class AlfredProgram {

    public virtual string HostConfigSection { get; } = "AppHost";
    public virtual string LoggingConfigSection { get; } = "Logging";
    public virtual string EnvironmentVariablePrefix { get; } = "ALFRED_";

    public int Run(string[] args) {
        IHostBuilder builder =
            Host.CreateDefaultBuilder(args)
                .ConfigureServices((context, services) => {
                    services.AddSingleton<IHost, AlfredHost>();
                    services.AddOptions<AlfredHostOptions>()
    ...
    protected int RunHost(IHostBuilder builder) {
        try {
            using AlfredHost host = (AlfredHost)builder.Build();
            host.Run();
